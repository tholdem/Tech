...
\begin{defn}
Let $ A$ be an abelian group and  $ k$ be a natural number. A \allbold{characteristic class of $ S_g$-bundles of degree  $ k$ with coefficients in  $ A$} is a natural transformation from the functors $ \Surf_g(-): \textsf{pchTop}^{op} \to \textsf{Set}$ to $ H^{k}(-;A): \textsf{pchTop}^{op} \to \textsf{Set}  $, where $ \textsf{pchTop} $ denotes the homotopy category of topological spaces with objects restricted to paracompact spaces (up to homotopy equivalence).
\end{defn}
Since any manifold is paracompact, we do not need to concern ourselves with the technicality of paracompactness.
...
In particular, when $ i+j =n$, then the intersection class is just an integer, which we call the intersection number.
...
When $ V$ is orientable, there exists an Euler class  $ e(V) \in H^{2}(E;\zz)$. For each $ i \in \nn$, we can take cup products of $ e(V)$ to obtain  $ e^{i+1}(V) \in H^{2i+2}(E;\zz)$. Therefore, when $ \dim  M = 2i$, $ e^{i+1}(V)$ lives in the top cohomology group of $ E$ and thus is an integer.  From the intersection perspective, since the Euler class represents the zero locus of a section $ E \to V$ (\emph{i.e.} a vertical vector field), $ e^{i+1}$ can be understood as the intersection number of the zero loci of $ i+1$ vertical vector fields. Since the zero section of $ V$ can be identified with $ E$, the intersection lives in $ E$ as well so it is represented by a class in $ H^{2i+2}(E;\zz)$.
...
\begin{defn}
	Define \allbold{the $ i$th Mumford-Miller-Morita class (MMM class)} to be $ e_i(E) = \pi_!\left( e^{i+1} \right) \in H^{2i}(M;\zz)$.
\end{defn}
\begin{prop}
...
Although for each $ i$, we have found a characteristic class $ e_i$ of surface bundles, this class is only interesting if it is not trivial, \emph{i.e.} corresponding to the zero class in $ H^{2i}(\Mod(S_g);\zz)$. Historically,  $ e_i$ is found to be nontrivial by a fact of Hirzebruch that for an oriented surface bundle $ E$ over an oriented closed surface $ M$, $ \langle e_i,[M] \rangle = 3 \sign(E)$, the signature of $ E$, and surface bundles with non-zero signatures have already been constructed by Atiya and Kodaira independently. We now present this construction.
...
Since any normal subgroup of $ \pi_1(M_2)$ gives a regular covering of $ M_2$, let $ (M_3, \rho_2)$ be such covering induced by the kernel $ K$ of the surjective map
\begin{align*}
	\pi_1(M_2) \twoheadrightarrow H_1(M_2; \zz) \twoheadrightarrow H_1(M_2;\zz /m).
\end{align*}
Since $ H_1(M_2 ; \zz) = \zz^{2g_2}$, $ H_1(M_2; \zz /m) \cong H_1(M_2;\zz) \otimes \zz /m$ by the universal coefficient theorem. Therefore, the first isomorphism theorem yields that $ [\pi_1(M_2):K] = |H_1(M_2;\zz /m)|= m^{2g_2}$. That is, $ M_3$ is a $ m^{2g_2}$-fold covering of $ M_2$. Then $ 2-2g_3 = m^{2g_2}(2-2g_2)$ which gives $ g_3 = 1+m^{2g_2}(g_2-1)$.
...
From now on, with an abuse of notation, we shall use the (co)homology class, its Poincar\'{e} dual, and the integer it represents (if applicable) interchangeably.
...
$ e_i(E) = \pi_!\left( e^{i+1}(V) \right)$. It suffices to show that $ e^{i+1}(E)$ is non-trivial.
...
\begin{align*}
	I \left( \widetilde{ D}, \widetilde{ D} \right) &= \left[ \widetilde{ D} \right] ^* \smile  \left[ \widetilde{ D} \right] ^*\\
							...
							&= \frac{1}{2m} f^* \left( [D]^*  \smile [D]^* \right)  && \text{ naturality} \\
							&= \frac{1}{2m} f^* I(D,D) \\
